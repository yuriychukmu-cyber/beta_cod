...
def main() -> None:
    ...
    if args.mock:
        ...
        try:
            ...
            base_used = base_url or os.getenv("DATSSOL_API_BASE_URL", "")
            print(f"[run_client] API request failed: {exc}", file=sys.stderr)
            if "/arena/arena" in str(exc) or base_used.rstrip("/").endswith("/arena"):
                print(
                    "[run_client] Hint: set DATSSOL_API_BASE_URL to API root "
                    "(e.g. https://games-test.datsteam.dev/api) or update to latest code with URL normalization.",
                    file=sys.stderr,
                )
            raise
